...
**最終更新**: 2026-05-27
**ステータス**: Phase N-5 着手前整合性対応（通知デモデータの共通モックデータ同期）完了 / Phase N-5（クロス画面フラッシュ）未着手
...
| N-5 クロスフラッシュ | 未着手 | — | — | 着手前整合性対応として、通知デモデータを共通モックデータ由来に同期済み（2026-05-27） |
| N-6 結合テスト | 未着手 | — | — | — |
...
**現在地**: N-4 ブラウザ確認完了 (2026-05-25)。OB/WS/LA/SL の **4 画面すべてが `(scope, op, opts)` 形式で動作中**。SL には応援タブ / 応援予約編集 / 応援D&D配置 / `support`・`reservation` 通知発信を追加済み。WS は `addPartner` / `deactivatePartner` 直接呼出通知、LA は `expandRecurrence` add 通知 / `onSaveLeave` メタ編集 modify 通知まで追加済み。N-4 では GC跨ぎ社員配置時の自動生成行データ (`isAutoGenerated/sourceSiteId/sourceEmployeeId`) と OB完全ロック表示、SL側削除同期を実画面で確認済み。OB表示では同一区分・昼夜・契約先名・業務名の自動生成受注を1行へ集約し、日付セルの人数/配置者をまとめる。4画面は `mock.oms.state.v1` の共通モックストアを使い、SLは日付別表状態、OBは月別受注状態、WSは週別予定状態、LAは休暇申請状態、GC跨ぎ自動生成行は共通ストア内 `autoGeneratedOrders` から読む。LAの休暇申請はWSの休み行へ投影する。デモ基準日は `demoToday/currentDate = 2026-05-01` に固定し、リセット後の初期表示・OB/WS/LAのToday判定・通知の「今日」ラベルを揃える。2026-05-27 に N-5 着手前整合性対応として、`co-navbar.js` と OB/SL/WS 画面の初期デモ通知を固定文言から共通モックデータ由来に変更済み。経理画面Dは未着手のため、今回はデータフラグ保持まで。

**次の確認ポイント**:

- **N-5 着手判断**: GC跨ぎ社員配置 → OB自動生成行作成 → 完全ロック表示 → SL側削除同期はユーザー確認済み。通知デモデータも共通モックデータ由来へ同期済み。次はクロス画面フラッシュ（`affects[]` / `target` 配線）へ進む。
