...
#### 3.7.9 通知カテゴリの導出ルール（2026-06-12 ユーザー採用）

SL/OB/WS/LA は同じデータを別の切り口で見る「窓」であり、**通知カテゴリは「どの画面で操作したか」ではなく「どのエンティティが変わったか」から導出する**。発信元画面はカテゴリではなく、ジャンプ先解決用メタデータ（`primaryPage`）として保持する。

| カテゴリ | 導出元エンティティ | 備考 |
| --- | --- | --- |
| 受注変更（order） | 受注行（daily_site_order 相当）の属性変更・追加・削除 | SL から編集しても OB から編集しても同一カテゴリ |
| 配置変更（assignment） | 人の配置（日別・週間）。応援・協力業者の配置を含む | サブタグ: 自社 / 応援 / 協力業者 / **車両・ETC**（車両配置・ETC割当・整備は当面このサブタグ。車両管理の専任担当を置く場合は独立カテゴリへ昇格を再検討） |
| 申請・承認（approval） | 休暇申請等の申請ワークフロー | 配置に影響するが「申請→承認」の別ライフサイクルのため独立維持 |
| マスタ・システム（master） | 社員・契約先・現場・資格・権限マスタ、システム通知 | 現行どおり |

**ファセット（カテゴリにしない横断属性）:**

- **対象日（影響する勤務日）** — 第一級ファセット。変更入力日時とは別に保持し、センターの「日別」軸は**対象日基準**でグループ化する（入力日順は時系列で足りる）。OB の日またぎ変更も対象日で正しい日に整理される
- 追加 / 変更 / 削除（既存タイプバッジ）
- 受領記録型 / 業務判断型（§3.7.2）
- 発信元画面（`primaryPage`）・影響画面（`affects[]`）

**運用ルール**: 通知は「事実の変更」のみに発火する。変更の結果生じる派生状態（人数不足等）は通知ではなく画面上の警告（info-pill 等）の責務とし、カテゴリにしない。

> モックアップへの適用は単独では行わず、`docs/plan/mockup-refactor-plan.md`（デザイン刷新との統合リファクタ）で一括実施する（2026-06-12 ユーザー指示）。
