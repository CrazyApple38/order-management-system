...
**現在地**: N-4 進行中 (2026-05-25)。OB/WS/LA/SL の **4 画面すべてが `(scope, op, opts)` 形式で動作中**。SL には応援タブ / 応援予約編集 / 応援D&D配置 / `support`・`reservation` 通知発信を追加済み。WS は `addPartner` / `deactivatePartner` 直接呼出通知、LA は `expandRecurrence` add 通知 / `onSaveLeave` メタ編集 modify 通知まで追加済み。N-4 では GC跨ぎ社員配置時の自動生成行データ (`isAutoGenerated/sourceSiteId/sourceEmployeeId`) と OB完全ロック表示、SL側削除同期を実装中。Playwright MCP で SL N-3.2 と WS addPartner 通知を確認済み。

**次の確認ポイント**:

- **N-4 ブラウザ確認**: GC跨ぎ社員配置 → OB自動生成行作成 → 完全ロック表示 → SL側削除同期を実画面で確認する。経理画面Dは未着手のため、今回はデータフラグ保持まで。
