...
- 新タブ遷移なら現在作業が失われない
- `affects[]` は「関連画面」、`target` は「実際にフラッシュ可能な画面」として分離する
...
| N-5 クロスフラッシュ | 完了 | 2026-05-27 | 2026-05-27 | 通知デモデータを共通モックデータ由来に同期後、同画面 `cn:jump` / 別画面 `cnJump` URL パラメータ / OB・SL・WS・LA 着地フラッシュを実装。追加で `domain` / `primaryPage` / 画面別 target 解決、SL差分の新旧表示、`admin-notify.html` の優先度ルール表を反映 |
| N-6 結合テスト | 未着手 | — | — | — |
