...
- モック実装では `mock.sl.autoGeneratedOrders.v1` に `isAutoGenerated/sourceSiteId/sourceEmployeeId` を保持し、OB側でロック行として読み込む
...
| N-4 自動受注生成 | 進行中 | 2026-05-25 | — | SL GC跨ぎ社員配置 → OB自動生成行 localStorage 連携 / OB完全ロック表示 / SL側削除同期を実装中 |
| N-5 クロスフラッシュ | 未着手 | — | — | — |
| N-6 結合テスト | 未着手 | — | — | — |
