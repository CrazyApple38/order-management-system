...
| 人物アイコンの角丸四角バッジ化 | **採用** | 配置バッジ内の人物アイコンを「所属色の角丸四角（20px・radius-xs）+ ピル表面色 `#eff3f7` で抜いた人物（辺の約55%・余白広め）」に変更。**内側パディング比率が要件**（サイズ変更時も比率維持）。SL実装済み、センター等の人物チップへも R-2/R-4 で展開 |
