...
function renderGrid() {
    ...
    sampleRows.forEach((row, ri) => {
        ...
        html += `<div class="tbl-grid__cell tbl-grid__sticky--0 tbl-grid__clickable ${rowCls}${evenCls}" data-ri="${ri}"${shiftAttr} onclick="openRowEditModal(${ri})">${obBranchShort(row.branch)}</div>`;
        html += `<div class="tbl-grid__cell tbl-grid__sticky--1 tbl-grid__clickable ${rowCls}${evenCls}" data-ri="${ri}"${shiftAttr} onclick="openRowEditModal(${ri})">${row.category}</div>`;
        html += `<div class="tbl-grid__cell tbl-grid__sticky--2 tbl-grid__clickable ${rowCls}${evenCls}" data-ri="${ri}"${shiftAttr} onclick="openRowEditModal(${ri})">${row.shift}</div>`;
        html += `<div class="tbl-grid__cell tbl-grid__sticky--3 tbl-grid__clickable ${rowCls}${evenCls}" data-ri="${ri}"${shiftAttr} onclick="openRowEditModal(${ri})">${row.company}</div>`;
        ...
        html += `<div class="tbl-grid__cell tbl-grid__sticky--5 tbl-grid__clickable ${rowCls}${evenCls}" data-ri="${ri}"${shiftAttr} onclick="openRowEditModal(${ri})">${taskLabel}</div>`;
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            if (entries.length === 1) {
                ...
                cellContent = entries.map((entry, si) => {
                    ...
                    return `<div class="tbl-grid__site-entry${confCls}" data-ri="${ri}" data-day="${d}" data-si="${si}" onclick="event.stopPropagation(); openCalendarWithEdit(${ri},${d},${si})" onmouseenter="showTooltip(event,${ri},${d},${si})" onmouseleave="hideTooltip()"><span class="tbl-grid__cell-count">${entry.count}</span>${subTaskHtml}${badgeHtml}</div>`;
                }).join('');
            }
            ...
            html += `<div class="${cls}" data-ri="${ri}" data-day="${d}"${dowAttr}${shiftAttr} ${cellHover} ${cellClick}>${cellContent}</div>`;
        }
...
function obCnOpenModalForRow(ri) {
    ...
    if (row) {
        var catClass = obCnCatClassMap[row.category] || 'md-cn-cat-facility';
        var shiftClass = obCnShiftClassMap[row.shift] || 'md-cn-shift-day';
        info.innerHTML =
...
function obCnJumpToCard(notificationId) {
    ...
    // カードにdata-nid属性で対象を探してスクロール＋ハイライト
    setTimeout(function() {
    ...
    list.innerHTML = filtered.map(function(n) {
            ...
            diffHtml = '<div class="md-cn-diff-list">' +
                n.diffs.map(function(d) {
                    return '<div class="md-cn-diff-row">' +
                        '<span class="md-cn-diff-label">' + escapeHtml(d.field) + '</span>' +
        ...
        if (n.reverted) {
            actionsHtml = '<div class="md-cn-card-actions">' +
                '<button class="md-cn-btn-reapprove" onclick="event.stopPropagation(); obCnReapprove(' + n.id + ')">適用する</button>' +
            '</div>';
        } else {
            actionsHtml = '<div class="md-cn-card-actions">' +
                '<button class="md-cn-btn-revert" onclick="event.stopPropagation(); obCnRevert(' + n.id + ')">キャンセル</button>' +
            '</div>';
        }
        ...
        if (isRowMode) {
            // 行ベルモード: 対象日のみ（あれば）
            if (dayLabel) {
                siteHtml = '<div class="md-cn-card-site">' +
                    '<span class="md-cn-day-badge">' + dayLabel + '</span>' +
                '</div>';
            }
        } else {
            // 全体モード: 会社・区分・契約先名・業務名・昼夜・対象日
            siteHtml = '<div class="md-cn-card-site">' +
                '<span class="md-cn-info-item">' + escapeHtml(obBranchShort(n.branch || '')) + '</span>' +
                '<span class="md-cn-category-badge ' + catClass + '">' + n.category + '</span>' +
                '<span class="md-cn-info-item">' + escapeHtml(n.company || '') + '</span>' +
                '<span class="md-cn-info-item">' + escapeHtml(displayName) + '</span>' +
                '<span class="md-cn-shift-badge ' + shiftClass + '">' + n.shift + '</span>' +
                (dayLabel ? '<span class="md-cn-day-badge">' + dayLabel + '</span>' : '') +
            '</div>';
        }

        return '<div class="' + cardClass + stateClass + '" data-nid="' + n.id + '" onclick="obCnCardClick(' + n.id + ')">' +
