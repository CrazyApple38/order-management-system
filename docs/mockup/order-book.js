...
// カレンダー専用 Undo/Redo は R-3b で撤去（中央ビュー化に伴いグローバル undo/redo へ一本化）
...
function renderGrid() {
    ...
    sampleRows.forEach((row, ri) => {
        ...
        if (row.isAutoGenerated) {
            ...
            html += `<div class="tbl-grid__cell tbl-grid__sticky--7 ${rowCls}${evenCls}" data-ri="${ri}"${shiftAttr}><button class="tbl-grid__cal-open-btn" onclick="event.stopPropagation(); obOpenRowCalendar(${ri})" title="行カレンダーを表示"><img src="mockup/icons/calendar.svg" alt="カレンダー"></button></div>`;
        }
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            if (entries.length === 1) {
                ...
                cellContent = entries.map((entry, si) => {
                    ...
                    const entryClick = row.isAutoGenerated ? '' : ` onclick="event.stopPropagation(); openEditModal(${ri},${d},${si})"`;
            ...
            const cellClick = (row.isAutoGenerated || entries.length > 1) ? '' : `onclick="openEditModal(${ri},${d},0)"`;
    ...
    obSaveMonthToCommonStore();
    // 行カレンダービュー表示中はデータ変更に追従して再描画（undo/redo・保存・月移動を一括カバー）
    if (obViewMode === 'calendar') renderCalendarGrid();
}

// --- フィルタ（ツールバー常設 seg + 検索カプセル / 03 §1.1 確定） ---

// 汎用: seg の選択値取得（data-all 以外の active ボタン）
function obSegSelected(segId) {
    var seg = document.getElementById(segId);
    if (!seg) return [];
    var btns = seg.querySelectorAll('button.active:not([data-all])');
    var arr = [];
    for (var i = 0; i < btns.length; i++) arr.push(btns[i].dataset.value);
    return arr;
}

// 汎用: 個別選択が無ければ「すべて」を点灯
function obSyncSegAll(seg) {
    var allBtn = seg.querySelector('[data-all]');
    if (!allBtn) return;
    var anyActive = seg.querySelector('button.active:not([data-all])');
    allBtn.classList.toggle('active', !anyActive);
}

// data-multi seg の切替（「すべて」と個別選択は排他）
function obInitFilterSegs() {
    ['obSegBranch', 'obSegCategory', 'obSegShift'].forEach(function(id) {
        var seg = document.getElementById(id);
        if (!seg || seg.dataset.obSegInit === '1') return;
        seg.dataset.obSegInit = '1';
...
// sampleRowsから会社 seg の選択肢を動的生成（選択状態は維持）
function buildBranchPanel() {
    var seg = document.getElementById('obSegBranch');
    if (!seg) return;
    var branches = [];
    ...
    branches.sort();
    var selected = obSegSelected('obSegBranch');
    seg.querySelectorAll('button:not([data-all])').forEach(function(b) { b.remove(); });
    branches.forEach(function(b) {
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.dataset.value = b;
        btn.textContent = b;
        if (selected.indexOf(b) !== -1) btn.classList.add('active');
        seg.appendChild(btn);
    });
    obSyncSegAll(seg);
}
...
function isFiltered(row) {
    ...
    const branches = obSegSelected('obSegBranch');
    const cats = obSegSelected('obSegCategory');
    const shifts = obSegSelected('obSegShift');
    const fcoEl = document.getElementById('filterCompany');
    const fco = fcoEl ? fcoEl.value.trim() : '';
    if (branches.length > 0 && branches.indexOf(row.branch) === -1) return false;
    if (cats.length > 0 && cats.indexOf(row.category) === -1) return false;
    if (shifts.length > 0 && shifts.indexOf(row.shift) === -1) return false;
...
function clearFilters() {
    ['obSegBranch', 'obSegCategory', 'obSegShift'].forEach(function(id) {
        var seg = document.getElementById(id);
        if (!seg) return;
        seg.querySelectorAll('button').forEach(function(b) { b.classList.toggle('active', b.hasAttribute('data-all')); });
    });
    var fco = document.getElementById('filterCompany');
    if (fco) fco.value = '';
    renderGrid();
}
...
function toggleWeekendColors() {
    ...
    const btn = document.getElementById('weekendColorsToggleBtn') || document.querySelector('.md-ob-tb-btn:nth-child(4)');
    if (btn) btn.classList.toggle('active', weekendColorsEnabled);
    renderGrid();
}
...
// カレンダー専用バッジトグルは R-3b で撤去（中央ビュー化に伴いツールバー「バッジ」に一本化）

// --- 表示/非表示トグル ---
function toggleHiddenRows() {
    showHiddenRows = !showHiddenRows;
    const btn = document.getElementById('hiddenRowsToggleBtn') || document.querySelector('.md-ob-tb-btn:nth-child(3)');
    if (btn) btn.classList.toggle('active', showHiddenRows);
    renderGrid();
}
...
function openEditModal(ri, day, siteIdx) {
    ...
    // 行カレンダービューの対象行を選択に追従させる
    if (!row.isAutoGenerated) calendarRi = ri;
    obDockEditModal(row, dateStr);
    if (obViewMode === 'calendar') renderCalendarGrid();
}
...
let obDockState = null;

function obRestoreDockedModal(kind) {
    if (!obDockState) return;
    if (kind && obDockState.kind !== kind) return;
    if (obDockState.headerPlaceholder && obDockState.headerPlaceholder.parentNode) {
        obDockState.headerPlaceholder.replaceWith(obDockState.header);
    }
    if (obDockState.bodyPlaceholder && obDockState.bodyPlaceholder.parentNode) {
        obDockState.bodyPlaceholder.replaceWith(obDockState.body);
    }
    if (obDockState.footerPlaceholder && obDockState.footerPlaceholder.parentNode) {
        obDockState.footerPlaceholder.replaceWith(obDockState.footer);
    }
    const dock = document.getElementById('obPropDetailDock');
    if (dock) { dock.innerHTML = ''; delete dock.dataset.docked; }
    const empty = document.getElementById('obPropDetailEmpty');
    if (empty) empty.style.display = '';
    obDockState = null;
    const activeBtn = document.querySelector('.panel-rail [data-panel].active');
    obUpdatePropHead(activeBtn ? activeBtn.dataset.panel : 'detail');
}

function obDockModal(kind, overlayId, title, subText) {
    obRestoreDockedModal();
    ...
    const headerPlaceholder = document.createComment('ob-docked-header');
    const bodyPlaceholder = document.createComment('ob-docked-body');
    const footerPlaceholder = document.createComment('ob-docked-footer');
    header.replaceWith(headerPlaceholder);
    body.replaceWith(bodyPlaceholder);
    footer.replaceWith(footerPlaceholder);

    dock.innerHTML = '';
    dock.appendChild(header);
    dock.appendChild(body);
    dock.appendChild(footer);
    dock.dataset.docked = kind;
    overlay.style.display = 'none';

    const empty = document.getElementById('obPropDetailEmpty');
    if (empty) empty.style.display = 'none';
    const titleEl = document.getElementById('obPropTitle');
    if (titleEl) titleEl.textContent = title || '詳細';
    const subEl = document.getElementById('obPropSub');
    if (subEl) subEl.textContent = subText || '';
    obSetPropMode('detail');

    obDockState = { kind, overlay, modal, header, body, footer, headerPlaceholder, bodyPlaceholder, footerPlaceholder, title: title || '詳細', sub: subText || '' };
    // 連携・所在／変更履歴パネルを選択行に追従
    if (editingRi !== null && sampleRows[editingRi]) obSelectedRi = editingRi;
    else if (editingRowRi != null && editingRowRi >= 0 && sampleRows[editingRowRi]) obSelectedRi = editingRowRi;
    obRefreshPropPanels();
}

function obDockEditModal(row, dateStr) {
    const task = row && row.task ? row.task : '(個別業務)';
    obDockModal('cell', 'editModalOverlay', 'セル編集', `${dateStr} / ${row.company || '(契約先未設定)'} / ${task}`);
}

function obDockRowEditModal(row) {
    const title = editingRowRi === -1 ? '行情報編集 - 新規追加' : '行情報編集';
    const sub = row
        ? `${row.company || '(契約先未設定)'} / ${row.task || '(業務名未設定)'}`
        : '新しい受注行';
    obDockModal('row', 'rowEditModalOverlay', title, sub);
}
...
// --- 行カレンダー（中央ビュー切替 / 03 §4 R-3b: カレンダー入力モーダル廃止） ---
// 表示月・undo/redo はツールバー共通（changeMonth / undo / redo）に追従。
// 編集は日セルクリック → openEditModal → 右プロパティ「詳細」ドック。
let obViewMode = 'grid'; // 'grid' | 'calendar'
let calendarRi = null;   // 行カレンダーの対象行（選択行に追従。未選択時は先頭表示行）

const obMainHintTexts = {
    grid: '月間グリッド。詳細・所在・履歴は右プロパティへ。',
    calendar: '行カレンダー。日セルを選択すると右プロパティで編集します。'
};

// 行カレンダーの対象にできる先頭の表示行（フィルタ適用後・自動生成行を除く）
function obFirstCalendarRow() {
    for (let ri = 0; ri < sampleRows.length; ri++) {
        const row = sampleRows[ri];
        if (!row || row.isAutoGenerated) continue;
        if (row.hidden && !showHiddenRows) continue;
...
function obSetView(mode) {
    const next = mode === 'calendar' ? 'calendar' : 'grid';
    if (next === 'calendar') {
        const row = calendarRi !== null ? sampleRows[calendarRi] : null;
        if (!row || row.isAutoGenerated || !isFiltered(row) || (row.hidden && !showHiddenRows)) {
            calendarRi = obFirstCalendarRow();
        }
        if (calendarRi === null) return; // 表示できる行が無い場合はビューを切り替えない
    }
    obViewMode = next;
    const gridBtn = document.getElementById('obViewGridBtn');
    const calBtn = document.getElementById('obViewCalBtn');
    if (gridBtn) gridBtn.classList.toggle('active', next === 'grid');
    if (calBtn) calBtn.classList.toggle('active', next === 'calendar');
    const gridFrame = document.getElementById('obGridFrame');
    const calFrame = document.getElementById('obCalFrame');
    if (gridFrame) gridFrame.style.display = next === 'grid' ? '' : 'none';
    if (calFrame) calFrame.style.display = next === 'calendar' ? '' : 'none';
    const hint = document.getElementById('obMainHint');
    if (hint) hint.textContent = obMainHintTexts[next];
    if (next === 'calendar') renderCalendarGrid();
}

// 行内カレンダーアイコン: その行を選択して行カレンダービューへ（旧 openCalendarModal の導線を継承）
function obOpenRowCalendar(ri) {
    const row = sampleRows[ri];
    if (!row || row.isAutoGenerated) return;
    calendarRi = ri;
    obSetView('calendar');
...
function renderCalendarGrid() {
    if (calendarRi === null || !sampleRows[calendarRi]) return;
    const daysInMonth = new Date(currentYear, currentMonth, 0).getDate();
    const firstDow = new Date(currentYear, currentMonth - 1, 1).getDay();
    const dayNames = ['日', '月', '火', '水', '木', '金', '土'];

    const rowData = cellData[calendarRi] || {};
    const row = sampleRows[calendarRi];

    // 対象行メタ（会社・区分・昼夜・契約先・業務名）
    const calMeta = document.getElementById('obCalMeta');
    if (calMeta) {
        const taskDisplay = row.task ? escapeHtml(row.task) : '<span class="md-ob-individual-task">(個別業務)</span>';
        calMeta.innerHTML =
            `<span class="md-ob-cal-meta-company">${escapeHtml(obBranchShort(row.branch))}</span>` +
    ...
    // セル編集ドックが対象行を編集中のときのみ選択ハイライト
    const editSelActive = obDockState && obDockState.kind === 'cell' && editingRi === calendarRi;
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (d === editingDay && editSelActive) cls += ' md-ob-cal-selected';
        ...
        if (entries.length > 0) {
            ...
            for (let si = 0; si < visibleCount; si++) {
                ...
                const isSelected = (d === editingDay && si === editingSiteIdx && editSelActive);
...
// 行カレンダーの日セルクリック: 右プロパティ「詳細」でセル編集（選択反映は openEditModal 側で再描画）
...
document.addEventListener('cn:jump', function (e) {
    ...
    if (ri < 0) return;
    // 着地行を右プロパティ「連携・所在」「変更履歴」に反映
    obSelectRow(ri);
    // 着地先は月間グリッドのセル。行カレンダービュー表示中はグリッドへ戻す
    if (obViewMode === 'calendar') obSetView('grid');
...
document.addEventListener('cn:action', function (e) {
    ...
    if (action === 'recover-order') {
        obDemoInsertRow();
        if (obViewMode === 'calendar') obSetView('grid'); // 復旧行の着地はグリッド
        renderGrid();
        var ri = obDemoRowIndex();
        if (ri >= 0) {
...
// ==================== 右プロパティ「連携・所在」「変更履歴」（R-3b 実データ配線） ====================

let obSelectedRi = null; // 最後に選択された行（セル編集・行編集・行カレンダーで更新）

function obGetSelectedRow() {
    return (obSelectedRi !== null && sampleRows[obSelectedRi]) ? sampleRows[obSelectedRi] : null;
}

function obSelectRow(ri) {
    if (ri == null || ri < 0 || !sampleRows[ri]) return;
    obSelectedRi = ri;
    obRefreshPropPanels();
}

function obPropRowLabel(row) {
    return `${row.company || '(契約先未設定)'} / ${row.task || '(個別業務)'}`;
}

function obUpdatePropHead(mode) {
    const titleEl = document.getElementById('obPropTitle');
    const subEl = document.getElementById('obPropSub');
    if (!titleEl || !subEl) return;
    if (mode === 'detail') {
        if (obDockState) {
            titleEl.textContent = obDockState.title || '詳細';
            subEl.textContent = obDockState.sub || '';
        } else {
            titleEl.textContent = '未選択';
            subEl.textContent = '行または日セルを選択すると、ここに内容を表示します。';
        }
        return;
    }
    const row = obGetSelectedRow();
    titleEl.textContent = row ? obPropRowLabel(row) : '未選択';
    subEl.textContent = mode === 'linkage'
        ? '請求先・所在・SL/WS 連携状況'
        : (row ? '選択行に関係する変更通知' : '受注簿に関する変更通知');
}

// --- 連携・所在（03 §4: 請求先・現場住所・地図URL・SL/WS 連携状況） ---

// 【モックアップ専用】SL 現場との対応は共通ソース createSiteOrderMap（会社+業務名）から逆引き。
// 本番では daily_site_orders と sites の外部キーで解決する。
function obFindLinkedSite(row) {
    if (!window.OmsMockAssignmentsData) return null;
    const map = window.OmsMockAssignmentsData.createSiteOrderMap();
    const siteId = Object.keys(map).find(function (id) {
        return map[id].company === row.company && map[id].task === row.task;
    });
    if (!siteId) return null;
    const site = (window.OmsMockAssignmentsData.createSites() || []).find(function (s) { return s.id === siteId; });
    return site || { id: siteId, name: map[siteId].task, gc: null };
}

// 共通配置ソースから対象現場の配置数（デモ今日 / 今週）を集計
function obCountSiteAssignments(siteId) {
    if (!window.OmsMockAssignmentsData) return { today: 0, week: 0 };
    const assignments = window.OmsMockAssignmentsData.createEmployeeAssignments();
    const todayKey = obGetDemoTodayKey();
    const monday = obGetDemoTodayDate();
...
// 表示月のセルデータから地図URLを集約（URL重複は除外）
function obCollectRowMaps(ri) {
    const rowData = cellData[ri] || {};
    const seen = {};
    const maps = [];
    Object.keys(rowData).forEach(function (day) {
        getCellEntries(ri, +day).forEach(function (entry) {
            const list = entry.maps || (entry.mapUrl ? [{ label: '現場地図', url: entry.mapUrl }] : []);
            (list || []).forEach(function (m) {
                if (!m || !m.url || seen[m.url]) return;
                seen[m.url] = true;
                maps.push({ label: m.label || '現場地図', url: m.url });
            });
        });
    });
    return maps;
...
function obRenderLinkagePanel() {
    const body = document.getElementById('obPropLinkageBody');
    const empty = document.getElementById('obPropLinkageEmpty');
    ...
    let html = '';
    html += '<div class="ob-lk-section">請求先・契約</div>';
    html += '<dl class="ob-lk-dl">';
    html += `<div><dt>会社</dt><dd>${escapeHtml(row.branch || '')}</dd></div>`;
    html += `<div><dt>契約先（請求先）</dt><dd>${escapeHtml(row.company || '(未設定)')}</dd></div>`;
    html += `<div><dt>業務名</dt><dd>${row.task ? escapeHtml(row.task) : '(個別業務)'}</dd></div>`;
    // 【モックアップ専用】現場住所はマスタ未整備（SHARED-MEMORY「未決の横断判断②」参照）
    html += '<div><dt>現場住所</dt><dd class="ob-lk-muted">未登録（現場マスタ未整備）</dd></div>';
    html += '</dl>';

    obLinkageMaps = obCollectRowMaps(obSelectedRi);
    html += '<div class="ob-lk-section">地図URL（表示月のセルから集約）</div>';
    if (!obLinkageMaps.length) {
        html += '<div class="ob-lk-muted">この行の表示月セルに地図URLはありません。</div>';
    } else {
        html += obLinkageMaps.map(function (m, i) {
            return `<div class="ob-lk-map">` +
                `<div class="ob-lk-map-head"><span class="ob-lk-map-label">${escapeHtml(m.label)}</span>` +
                `<button type="button" class="btn btn-sm btn-outline" onclick="obToggleLinkageMapPreview(${i})">プレビュー</button></div>` +
                `<div class="ob-lk-map-url">${escapeHtml(m.url)}</div>` +
                `<div class="ob-lk-map-preview" id="obLkMapPreview${i}" style="display:none;"></div>` +
            `</div>`;
        }).join('');
    }

    html += '<div class="ob-lk-section">SL/WS 連携状況</div>';
    if (row.isAutoGenerated) {
        html += '<div class="ob-lk-link">SL配置から自動生成されたロック行です。</div>';
    }
    const site = obFindLinkedSite(row);
    if (site) {
        const counts = obCountSiteAssignments(site.id);
        html += '<dl class="ob-lk-dl">';
        html += `<div><dt>SL 現場</dt><dd>${escapeHtml(site.name)}（${escapeHtml(site.id)}）</dd></div>`;
        html += `<div><dt>本日の配置</dt><dd>${counts.today} 名</dd></div>`;
        html += `<div><dt>今週の配置</dt><dd>${counts.week} 件</dd></div>`;
        html += '</dl>';
    } else if (!row.isAutoGenerated) {
        html += '<div class="ob-lk-muted">SL/WS の現場と対応付けられていません。</div>';
    }
    body.innerHTML = html;
}

// 地図プレビュー（右プロパティ288px内の iframe 埋め込み / 03 §4 R-3b 検証事項）
function obToggleLinkageMapPreview(idx) {
    const box = document.getElementById('obLkMapPreview' + idx);
    const m = obLinkageMaps[idx];
    if (!box || !m) return;
...
// --- 変更履歴（03 §4: 行/セルの変更履歴 = 通知ストア getItems('all') 由来 / SL R-3a-3 と同型） ---

function obIsOrderBookNotifyItem(item) {
    if (!item) return false;
    if (item.sourceBell === 'ob' || item.primaryPage === 'order-book') return true;
    if (Array.isArray(item.affects) && item.affects.indexOf('order-book') !== -1) return true;
    const t = item.target || null;
    if (t && t['order-book'] && t['order-book'].axis) return true;
    return !!(t && t.axis === 'orderId');
}

function obNotifyTargetForScreen(item) {
    const t = item && item.target;
    if (!t) return null;
    if (t['order-book']) return t['order-book'];
    return t.axis ? t : null;
}

function obNotifyMatchesRow(item, row) {
    if (!row) return false;
    const t = obNotifyTargetForScreen(item);
    if (!t) return false;
    if (t.axis === 'orderId') return String(t.value) === String(row._rowId);
    if (t.axis === 'siteName') {
        const v = String(t.value || '');
        const task = String(row.task || '');
        return !!(task && v && (task.indexOf(v) >= 0 || v.indexOf(task) >= 0));
    }
    return false;
}

function obNotifyItems() {
    if (!window.coNotifyPanel || typeof window.coNotifyPanel.getItems !== 'function') return [];
    return window.coNotifyPanel.getItems('all').filter(obIsOrderBookNotifyItem);
}

function obRenderHistoryPanel() {
    const scope = document.getElementById('obPropHistoryScope');
    const listEl = document.getElementById('obPropHistoryList');
    if (!scope || !listEl) return;
    const row = obGetSelectedRow();
    const items = obNotifyItems();
    const filtered = row ? items.filter(function (item) { return obNotifyMatchesRow(item, row); }) : items;
    const list = filtered.slice(0, 8);
    scope.textContent = row
        ? `対象行: ${obPropRowLabel(row)}（${filtered.length}件）`
        : `受注簿に関する通知（最新${list.length}件）`;
    if (!list.length) {
        listEl.innerHTML = `<div class="ob-prop-empty">${row ? 'この行に関する通知はまだありません。' : '受注簿に関する通知はまだありません。'}</div>`;
        return;
    }
    listEl.innerHTML = list.map(function (item) {
        ...
        const diffHtml = firstDiff
            ? `<span class="ob-history-diff">${escapeHtml(firstDiff.field || '変更')}: ${escapeHtml(firstDiff.oldVal || '(空)')} → ${escapeHtml(firstDiff.newVal || '(空)')}</span>`
            : '';
        const meta = [item.date, item.sub].filter(Boolean).join(' / ');
        return `<button type="button" class="ob-history-item" data-cn-history-id="${escapeHtml(item.id || '')}">` +
            `<span class="ob-history-main">${escapeHtml(item.main || '')}</span>` +
            diffHtml +
            (meta ? `<span class="ob-history-meta">${escapeHtml(meta)}</span>` : '') +
            `</button>`;
    }).join('');
}

// 履歴項目クリック → 通知パネルの該当項目クリックに委譲（SL R-3a-3 と同型）。無ければ target で直接着地
function obBindHistoryPanel() {
    const listEl = document.getElementById('obPropHistoryList');
    ...
    listEl.addEventListener('click', function (e) {
        ...
        const cnItem = Array.prototype.slice.call(document.querySelectorAll('.cn-item[data-id]'))
            .find(function (el) { return el.dataset.id === id; });
        const rowEl = cnItem ? cnItem.querySelector(':scope > .cn-item-row') : null;
        if (rowEl) { rowEl.click(); return; }
        const item = obNotifyItems().find(function (i) { return String(i.id) === id; });
        const t = item ? obNotifyTargetForScreen(item) : null;
        if (t) document.dispatchEvent(new CustomEvent('cn:jump', { detail: { inContext: true, target: t, op: item.op || 'modify' } }));
    });
}

// 通知の追加/削除に履歴パネルを追従させる（SL R-3a-3 と同型のフック）
function obPatchNotifyRefreshHooks() {
    if (!window.coNotifyPanel || window.coNotifyPanel._obHistoryHooked) return;
    ['setItems', 'addItem', 'removeItem', 'clearItems'].forEach(function (name) {
        ...
        window.coNotifyPanel[name] = function () {
            ...
            setTimeout(obRenderHistoryPanel, 0);
            return result;
        };
    });
    window.coNotifyPanel._obHistoryHooked = true;
}

function obRefreshPropPanels() {
    obRenderLinkagePanel();
    obRenderHistoryPanel();
    const activeBtn = document.querySelector('.panel-rail [data-panel].active');
    obUpdatePropHead(activeBtn ? activeBtn.dataset.panel : 'detail');
}

const obPropModeLabels = {
    detail: '詳細',
    linkage: '連携・所在',
    history: '変更履歴'
};

function obSetPropMode(mode) {
    const nextMode = obPropModeLabels[mode] ? mode : 'detail';
    document.querySelectorAll('.ob-prop-panel').forEach(function(panel) {
        panel.classList.toggle('is-active', panel.dataset.propPanel === nextMode);
    });
    document.querySelectorAll('.panel-rail [data-panel]').forEach(function(btn) {
        btn.classList.toggle('active', btn.dataset.panel === nextMode);
    });
    const label = document.getElementById('obPropModeLabel');
    if (label) label.textContent = obPropModeLabels[nextMode];
    if (nextMode === 'linkage') obRenderLinkagePanel();
    if (nextMode === 'history') obRenderHistoryPanel();
    obUpdatePropHead(nextMode);
}

function obMountNotifyRail() {
    const rail = document.querySelector('.ob-workspace .rail');
    const bells = document.getElementById('mdNavCnBells');
    if (!rail || !bells || bells.dataset.obRailMounted === '1') return;
    bells.dataset.obRailMounted = '1';
    bells.classList.add('ob-rail-cn-bells');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    obSetPropMode('detail');
    obMountNotifyRail();
    setTimeout(obMountNotifyRail, 0);
    // 変更履歴パネル: クリック委譲 + 通知追加/削除フック + 初期描画
    obBindHistoryPanel();
    obPatchNotifyRefreshHooks();
    setTimeout(obRefreshPropPanels, 0);
});
