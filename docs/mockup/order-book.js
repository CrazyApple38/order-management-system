...
function openEditModal(ri, day, siteIdx) {
    ...
    if (!calEditPanelActive) obDockEditModal(row, dateStr);
}
...
function obDockModal(kind, overlayId, title, subText) {
    ...
    obDockState = { kind, overlay, modal, header, body, footer, headerPlaceholder, bodyPlaceholder, footerPlaceholder };
}
...
function openCalendarWithEdit(ri, day, siteIdx) {
    ...
    const isCalendarOpen = overlay && getComputedStyle(overlay).display !== 'none';
    if (!isCalendarOpen) {
        openEditModal(ri, day, siteIdx);
        return;
    }
    // カレンダー表示中は従来どおり、カレンダー内クリックと同じ扱いにする。
