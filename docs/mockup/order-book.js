...
function obApplyAutoGeneratedOrders() {
    ...
    orders.forEach(order => {
        ...
        const row = {
            _rowId: 'auto:' + order.id,
            branch: order.branch || order.employeeGcName || '',
            category: order.category || '応援交通',
            shift: order.shift || '昼',
            company: order.company || '',
            task: order.task || '',
            hidden: false,
            isAutoGenerated: true,
            sourceSiteId: order.sourceSiteId || '',
            sourceEmployeeId: order.sourceEmployeeId || '',
        ...
        if (m && parseInt(m[1], 10) === currentYear && parseInt(m[2], 10) === currentMonth) {
...
window.addEventListener('storage', function(e) {
    if (e.key !== OB_AUTO_ORDER_STORAGE_KEY) return;
    obApplyAutoGeneratedOrders();
    applyAutoConfidence();
    buildBranchPanel();
    renderGrid();
