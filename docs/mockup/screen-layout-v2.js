        ...
        const employeeContactItems = [
            { name: '会社', bg: 'rgba(174,192,194,0.2)', color: '#AEC0C2', borderColor: 'rgba(174,192,194,0.4)', cssClass: 'contact-company' },
            { name: '直', bg: 'rgba(72,187,120,0.2)', color: '#68d391', borderColor: 'rgba(72,187,120,0.4)', cssClass: 'contact-direct' },
            { name: 'LINE', bg: 'rgba(56,178,172,0.2)', color: '#4fd1c5', borderColor: 'rgba(56,178,172,0.4)', cssClass: 'contact-line' },
            { name: '迎え', bg: 'rgba(254,243,199,0.15)', color: '#fbd38d', borderColor: 'rgba(251,211,141,0.4)', cssClass: 'contact-pickup' },
            { name: 'OP', bg: 'rgba(183,148,244,0.15)', color: '#b794f4', borderColor: 'rgba(183,148,244,0.4)', cssClass: 'contact-op' }
        ];

        // 動的カラーパレット（新規追加用・ダークテーマ）
        const colorPalette = [
            { bg: 'rgba(237,100,166,0.15)', color: '#ed64a6', borderColor: 'rgba(237,100,166,0.4)' },
            { bg: 'rgba(99,179,237,0.15)', color: '#63b3ed', borderColor: 'rgba(99,179,237,0.4)' },
            { bg: 'rgba(237,137,54,0.15)', color: '#ed8936', borderColor: 'rgba(237,137,54,0.4)' },
            { bg: 'rgba(72,187,120,0.15)', color: '#68d391', borderColor: 'rgba(72,187,120,0.4)' },
            { bg: 'rgba(183,148,244,0.15)', color: '#b794f4', borderColor: 'rgba(183,148,244,0.4)' }
        ];
        ...
        function getContactStyle(contactName) {
            ...
            return { className: 'contact-badge', bg: 'rgba(174,192,194,0.15)', color: '#AEC0C2' };
        }
            ...
            box.querySelectorAll('.box-section').forEach(section => {
                ...
                if (label && value) {
                    vtItems.push({
                        ...
                        bg: value.style.background || 'rgba(174,192,194,0.15)',
                        color: value.style.color || '#AEC0C2'
                    });
                }
            });
            renderVtItems();
        ...
        function addVtEmptyItem() {
            syncVtItemsFromDom();
            vtItems.push({ label: '', value: '', bg: 'rgba(174,192,194,0.15)', color: '#AEC0C2' });
            renderVtItems();
            const rows = document.querySelectorAll('#vtModalItems .vt-item-row');
            const lastRow = rows[rows.length - 1];
        ...
        const COLOR_DEFAULTS = {
            '--gc-color-touo': '#5b9bd5',
            '--gc-color-nikkei': '#e07070',
            '--cat-bg-facility': 'rgba(72, 187, 120, 0.2)',
            '--cat-bg-event': 'rgba(237, 137, 54, 0.2)',
            '--cat-bg-traffic': 'rgba(99, 179, 237, 0.2)',
            '--cat-bg-highway': 'rgba(183, 148, 244, 0.2)',
            '--cat-text-facility': '#68d391',
            '--cat-text-event': '#ed8936',
            '--cat-text-traffic': '#63b3ed',
            '--cat-text-highway': '#b794f4',
            '--shift-bg-day': 'rgba(236, 201, 75, 0.2)',
            '--shift-text-day': '#ecc94b',
            '--shift-bg-night': 'rgba(43, 108, 176, 0.35)',
            '--shift-text-night': '#90cdf4'
        };
        const MAX_PRESETS = 5;
        const STORAGE_KEY = 'colorPresets_v2_dark';
        const ACTIVE_PRESET_KEY = 'activeColorPreset_v2_dark';
        ...
        function checkConflict(notification) {
            ...
            // デモ用: 現場詳細モーダルが開いていれば常にコンフリクト発生とみなす
            return true;
        }
