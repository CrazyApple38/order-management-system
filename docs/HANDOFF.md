...
- **日付**: 2026-06-03
- **コミット**: d1701b6（直前 HEAD。本コミットで「業務管理計画書」見出しの太さ調整を反映し push 実施）
...
- (Claude Code・試行/不採用) 所属色バッジの「下から光が漏れる／ネオン発光」表現を複数方式（`box-shadow` カラーグロー / `::after` の `radial-gradient` 放射光 / 多重 `box-shadow` ネオン）で試したが、いずれも意図に合わず**ドット表示へ差し戻し**。所属色の見せ方は当面**ドット**で確定（同じ試行の繰り返しは不要）。
