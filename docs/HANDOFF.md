...
- (Claude Code) **所属会社カラーを参考画像基調の12色へ刷新（2026-06-12 ユーザー指示）**: 配送ダッシュボード参考画像の四角アイコン背景色 4色（Graphite `#23262B` / Teal `#2FAE9C` / Violet `#7D6CE8` / Amber `#F0B440`）を基調に、明/基準/濃の3トーン×4色=12色パレットへ（旧 Flexoki 16色から置換）。初期割当 A=Graphite / B=Teal / C=Violet / D=Amber / E=Teal Dark。スウォッチUIは12列1行に変更。区分円（GC=belong-1〜3参照）も連動: 施設=黒/交通=ティール/イベント=バイオレット。※Amber は警告橙 `#F15E2A` と色相が近めの点は要観察。
