...
- **日付**: 2026-06-29
- **コミット**: 52df8e6（直前 HEAD。本更新は同期確認のため SHA/日付を最新化）

## 直前にやったこと

- (Claude Code) **プロジェクトのスクリーンショット群とドキュメント資産 SVG を追加（2026-06-14、コミット `52df8e6`）**。`bf848e4` の後に積まれていた `6c66d57`（SL色・ツールチップ・バッジ調整＝下記エントリ）／`52df8e6` が HANDOFF ヘッダー（旧 `bf848e4`）に未反映だったため、本更新で SHA/日付を最新化し記録を同期。Git 作業ツリーはクリーン・`origin/master` と完全一致。
- (Claude Code) **SLモック 色・ツールチップ・バッジ微調整（2026-06-14）**: ①Belonging Company Colors の8色を**白45%混の淡いパステル**へ（`belongPalette` + `:root` の belong-1〜5 フォールバック整合）。②区分円の区分名を白→**ダークグレー `#3d4a57`**（淡色背景での可読性確保）。③**会社名(GC)ポップアップを契約先/現場名セル→区分セルのホバーへ移設**（`.category-cell[data-gc]::after`、区分円の**上方**表示で契約先/現場名に被らせない。3行の区分セルに `data-gc` 付与。site-cell の `data-gc` はプロパティパネルJS用に保持）。④資格者不足/地図未設定/OB更新/過多の**青バッジをブルーグレー**へ（`--info-text/bg/border` を配置バッジの青灰系=`#50616f`/`#eef2f6`/`#c9d5de` に変更）。「不足」(`count-state.short`)は警告オレンジ維持。※「区分円アイコンを拡大＋区分名を円下端へ」は試作後ユーザー判断で**差し戻し**（現行=アイコン16px・区分名は上に通常配置）。Playwright 検証済み（console error 0）。
...
- **要件↔DB 整合課題2件（2026-06-30 / NotebookLM横断レビュー→現物照合で確定・要判断）**: ①**顧客側金額** — 要件3.1.1は「単価/請求額/支払条件」を要求するが `daily_site_orders` に該当列なし（`purchase_orders`＝協力業者側・`intergroup_billings`＝GC間のみ）。顧客請求をシステムで保持するか/保持先を要判断。②**契約先連絡先** — `companies` は 2026-02-04 に住所/phone/email/contact_person 削除済だが要件3.1.1は契約先「連絡先・住所」を要求（`group_companies.phone` は注文書甲情報で別物）。要件を更新するか `companies` に戻すか要判断。**いずれも「要件 vs DB のどちらを正とするか」の判断待ち**。※同レビューの他3件（既定時刻の格納先＝「本実装フェーズで確定」と明記／カスタム連絡タイプ＝マスタを持たない設計意図／`workedPrevNight`＝モック専用・実装時に導出置換と明記）は対応不要と確認済。経緯は Claude メモリ `project_notebooklm_loop_oms.md` 反復#3。
