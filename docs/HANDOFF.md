...
- (Claude Code) **配置バッジの人物アイコンを角丸四角バッジ化（2026-06-12 ユーザー指示）**: 所属色ベタの角丸四角 + 人物アイコンをピル表面色 `#eff3f7` で抜く打ち抜き表現。サイズは 20px で作成後、**ユーザー指示で 1.35倍の 27px に拡大**（アイコン15px・余白比率55%維持、ピル高さ約37pxに成長）。**内側パディングが要件**（参考画像準拠でアイコン=辺の約55%・余白広め、ユーザー確認済み: 余白比率/20px維持/角丸4px）。SLモックの `.person-icon` を変更（右プロパティの社員候補チップにも自動適用）。センターモックの人物チップ（15px）への展開は R-2/R-4 で実施。記録: mockup-refactor-plan §2.1。
