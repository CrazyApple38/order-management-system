...
- N-5 クロス画面フラッシュを実装: 同画面 `cn:jump` 即時フラッシュ、別画面ボタンの新タブ遷移、`cnJump` URL パラメータ着地、OB/SL/WS/LA リスナーを接続
- `node --check` と XAMPP `localhost` Playwright で OB/SL/WS/LA の着地フラッシュを確認
