...
- 新モック `docs/preview/change-notification-center-mockup.html`（変更通知センター）をユーザー目視で確認し、フィードバックを反映する。次の判断ポイント: 軸タブの並び/種類の過不足、リストカードの情報量、解消記録の構造化フィールド（採用値/根拠/確認先/方法）の入力UI、通知パネル（第1層）からセンターへの導線。
- 変更通知・スレッド・経路関係者の **DB設計追補**（`docs/03_データベース設計.md` にテーブル4本相当が未反映: change_notifications / threads / thread_comments / route_participants 相当）。
- 第1層（通知パネル）の整理: パネル内検索の撤去、要返信バッジの意味変更（自分宛確認依頼のみ）。第3層（SL右プロパティ）のスレッドモード追加はセンター確定後。
