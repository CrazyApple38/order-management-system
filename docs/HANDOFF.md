...
- SL モックの**レール通知カード**（ベル → スライドカード → 該当セル選択+フラッシュ）をユーザー目視で確認し、フィードバック反映。位置/幅/件数バッジ/フラッシュの強さが調整候補。
- `change-notification-center-mockup.html`（センター）の**スレッド領域を簡素化モデルに合わせて改修**: スレッド/確認依頼/解消記録の UI を外し、詳細パネルは「変更内容 + 情報経路 + 現場画面で開く」中心へ。軸タブ・グループ化リストは維持。
- 変更通知・経路関係者の **DB設計追補**（`docs/03_データベース設計.md` 未反映: change_notifications / route_participants 相当。スレッド系テーブルは将来拡張のため設計メモのみ）。
