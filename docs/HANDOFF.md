...
- **更新者**: Codex (GPT-5)
- **日付**: 2026-06-10
- **コミット**: 9df9cfb（直前 HEAD。本「変更通知の情報照合要件反映」作業前の HEAD）
...
- 変更通知の情報照合要件を引き続きユーザーと壁打ちする。次の主要論点は、(1) 食い違いを手動で開始する条件と自動検知範囲、(2) 照合状態の正式名称と遷移、(3) 誰を確認依頼先として選べるか、(4) 現行情報を切り替える操作と履歴の残し方、(5) SL/WSの判断理由をどの操作で要求するか。
- 追加モック `docs/preview/change-notification-thread-mockup.html` をユーザー目視で確認し、変更確認スレッドを **SL右プロパティ内の1モードに統合するか / 左メニューから通知専用画面として開くか / 両方を併用するか** を判断する。あわせて「理由カテゴリ」「確認ステータス」「確認依頼ボタン」「電話済み記録」を正式要件へ入れるか確認する。
