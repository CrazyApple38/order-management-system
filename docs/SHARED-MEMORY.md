...
- **（R-3e 2026-07-10 更新）QA は統合ベル + 共通 `.cn-card` へ移行済み**（旧 `.cn-panel` マークアップと co-notify-panel.js の QA モバイル互換ハンドラは撤去）。QA のベルアンカーは **DOM 1個のみ**で、画面切替時に `qaCnMountAnchor()` がホーム⇄カレンダーのヘッダーへ移設する（共通 `getAnchor()`/`renderBellLatest()` が first-match 前提のため。アンカーを複製しないこと）。旧 `.cn-panel`/`.cn-history-*`/`.cn-pick-*` **CSS** は co-notify-panel.css に残置（利用者ゼロ・撤去は R-3f #4）。
