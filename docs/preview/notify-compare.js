...
(function () {
    ...
    /* N-3.4.2 (2026-05-21): ユーザー編集後の JSON をそのまま既定値として反映。
       語順統一 (場所が前) / 「編集→変更」「解除→削除」など語感整理 +
       sl-employee-place は color:secondary (橙) 固定。 */
    var N34_TYPES = [
        /* ===== OB (受注簿) ===== */
        { id: 'ob-row-add',     bell: 'ob', scope: 'row',   op: 'add',
          label: '行追加', template: '{company} / {task} 行を追加',
        ...
        { id: 'ob-row-delete',  bell: 'ob', scope: 'row',   op: 'delete',
          label: '行削除', template: '{company} / {siteName} 行を削除',
          trigger: '行削除ボタン', fields: '行全体' },
        { id: 'ob-site-add',    bell: 'ob', scope: 'site',  op: 'add',
          label: '受注追加', template: '{company} / {task}({day}日) に受注追加',
        ...
        { id: 'ob-site-modify', bell: 'ob', scope: 'site',  op: 'modify',
          label: '受注編集', template: '{company} / {task}({day}日) の受注変更',
        ...
        { id: 'ob-site-delete', bell: 'ob', scope: 'site',  op: 'delete',
          label: '受注削除', template: '{company} / {task}({day}日) の受注削除',
        ...
        { id: 'sl-row-modify',   bell: 'sl', scope: 'row',   op: 'modify',
          label: '行編集 (行メタ)', template: '{company} / {siteName} の {fields} を変更',
        ...
        { id: 'sl-employee-place',  bell: 'sl', scope: 'employee', op: 'place', color: 'secondary',
          label: '社員配置', template: '{siteLabel} に {empName} を配置',
        ...
        { id: 'sl-support-remove', bell: 'sl', scope: 'support', op: 'remove',
          label: '応援解除', template: '{siteLabel} から {supportName} を削除',
          trigger: '(N-3.2 で実装)',
          fields: 'supportAssignments[partnerId][dateKey] からの除去' },
        { id: 'sl-support-modify', bell: 'sl', scope: 'support', op: 'modify',
          label: '応援編集', template: '{siteLabel} {supportName} の {fields} を変更',
        ...
        { id: 'sl-reservation-modify', bell: 'sl', scope: 'reservation', op: 'modify',
          label: '応援予約編集', template: '{partnerName} の {day} 予約を変更',
          trigger: '(N-3.2 で実装)',
        ...
        { id: 'ws-schedule-add',    bell: 'ws', scope: 'schedule', op: 'add',
          label: '配置追加', template: '{siteName}({day}) に {empOrVehicleName} を配置',
        ...
        { id: 'ws-schedule-modify', bell: 'ws', scope: 'schedule', op: 'modify',
          label: '配置変更', template: '{srcSite}({srcDay}) → {dstSite}({dstDay}) に {empOrVehicleName} を移動',
        ...
        { id: 'ws-schedule-delete', bell: 'ws', scope: 'schedule', op: 'delete',
          label: '配置削除', template: '{siteName}({day}) から {empOrVehicleName} を削除',
        ...
        { id: 'la-application-add',     bell: 'la', scope: 'application', op: 'add',
          label: '申請追加', template: '{empName} が {day} {kind}({partition}) に休暇申請',
        ...
        { id: 'la-application-modify',  bell: 'la', scope: 'application', op: 'modify',
          label: '申請編集', template: '{empName} の {day} {fieldLabel} 申請を変更',
        ...
        { id: 'master-modify', bell: 'master', scope: '_legacy', op: 'modify',
          label: 'マスタ編集', template: '{masterType}: {itemName} を変更',
